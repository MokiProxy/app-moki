...
<div class="row">
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <form action="{{ route('dokter.vendors.update', $vendor->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $vendor->name) }}" required maxlength="255">
                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-bold">Deskripsi</label>
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" maxlength="1000">{{ old('description', $vendor->description) }}</textarea>
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="document_type_ids[]" class="form-control select2-init @error('document_type_ids') is-invalid @enderror" multiple required style="width: 100%;">
                                ...
                                <option value="{{ $dt->id }}" {{ in_array($dt->id, old('document_type_ids', $vendor->documentTypes->pluck('id')->toArray())) ? 'selected' : '' }}>
                                    {{ $dt->name }}
                                </option>
                                @endforeach
                            </select>
