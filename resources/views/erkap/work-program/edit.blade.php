...
$currentRiskIdentification = optional($workProgram->riskIdentification);
$currentDeptTarget = optional($currentRiskIdentification->departmentTarget);
$currentRatingCriteria = optional($currentDeptTarget->ratingCriteria);
    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.work-programs.update', $workProgram->id) }}" method="POST" id="form-work-program">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_identification_id" id="risk-identification-select" class="form-select @error('erkap_risk_identification_id') is-invalid @enderror" required>
                                ...
                                @foreach($riskIdentifications as $riskIdentificationOption)
                                    <option value="{{ $riskIdentificationOption->id }}" {{ old('erkap_risk_identification_id', $workProgram->erkap_risk_identification_id) == $riskIdentificationOption->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="border rounded p-3 bg-light" id="sasaran-rating-info">
                                <div class="row g-2">
                                    <div class="col-6">
                                        <small class="text-muted text-uppercase fw-bold">Sasaran</small>
                                        <div class="fw-semibold" id="info-target">{{ $currentDeptTarget->target ?? '-' }}</div>
                                    </div>
                                    <div class="col-3">
                                        <small class="text-muted text-uppercase fw-bold">Rating</small>
                                        <div><span class="badge bg-primary" id="info-rating">{{ $currentRatingCriteria->rating ?? '-' }}</span></div>
                                    </div>
                                    <div class="col-3">
                                        <small class="text-muted text-uppercase fw-bold">Kualifikasi</small>
                                        <div class="fw-semibold" id="info-qualification">{{ $currentRatingCriteria->qualification ?? '-' }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" min="0" max="100" name="year_plan" id="input-year-plan" class="form-control @error('year_plan') is-invalid @enderror" value="{{ old('year_plan', $workProgram->year_plan) }}" required>
                                <span class="input-group-text">%</span>
                            </div>
                            @error('year_plan') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            <div class="form-text">Masukkan persentase target tahunan (0–100%).</div>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-2">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" min="0" max="100" name="{{ $field }}" class="form-control monthly-plan @error($field) is-invalid @enderror" value="{{ old($field, $workProgram->{$field}) }}" required>
                                <span class="input-group-text">%</span>
                            </div>
                    ...
                    <div class="mt-3">
                        <div class="alert alert-light border mb-0">
                            <i class="mdi mdi-information-outline me-1"></i>Masukkan persentase rencana setiap bulan secara manual. Jumlahnya harus sama dengan rencana tahunan.
                        </div>
                    </div>
    ...
    $(document).ready(function() {
        ...
        function monthlySum() {
            return months.reduce(function (total, month) {
                var value = parseFloat($('input[name="' + month + '"]').val());
        ...
        $('#form-work-program').on('submit', function(e) {
            ...
            var monthlyPlanSum = monthlySum();
            var id = $('#risk-identification-select').val();
            var info = id ? riskInfoMap[id] : null;
            var invalidMonth = months.some(function(month) {
                var value = parseFloat($('input[name="' + month + '"]').val());
            ...
            if (isNaN(yearPlan) || yearPlan < 0 || yearPlan > 100 || invalidMonth) {
                e.preventDefault();
                Swal.fire('Gagal', 'Rencana tahunan dan bulanan harus berupa persentase antara 0 sampai 100.', 'error');
                return false;
            }

            if (Math.abs(monthlyPlanSum - yearPlan) > 0.01) {
                e.preventDefault();
                Swal.fire('Gagal', 'Jumlah persentase rencana bulanan harus sama dengan rencana tahunan.', 'error');
                return false;
            }
        });
