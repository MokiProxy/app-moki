    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.risk-identifications.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="erkap_department_target_id" class="form-select @error('erkap_department_target_id') is-invalid @enderror" required>
                                ...
                                @foreach($departmentTargets as $departmentTarget)
                                    <option value="{{ $departmentTarget->id }}" {{ old('erkap_department_target_id') == $departmentTarget->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_type_id" class="form-select @error('erkap_risk_type_id') is-invalid @enderror" required>
                                <option value="" disabled selected>Pilih Risk Type</option>
                                @foreach($riskTypes as $riskType)
                                    <option value="{{ $riskType->id }}" {{ old('erkap_risk_type_id') == $riskType->id ? 'selected' : '' }}>
                                        {{ $riskType->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_taxonomy_id" class="form-select @error('erkap_risk_taxonomy_id') is-invalid @enderror" required>
                                <option value="" disabled selected>Pilih Risk Taxonomy</option>
                                @foreach($riskTaxonomies as $riskTaxonomy)
                                    <option value="{{ $riskTaxonomy->id }}" {{ old('erkap_risk_taxonomy_id') == $riskTaxonomy->id ? 'selected' : '' }}>
                                        {{ $riskTaxonomy->name }}
                                    </option>
                                @endforeach
                            </select>
