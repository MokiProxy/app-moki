    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.risk-analysis.update', $riskAnalysis->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_identification_id" class="form-select @error('erkap_risk_identification_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_risk_identification_id', $riskAnalysis->erkap_risk_identification_id) ? '' : 'selected' }}>Pilih Identifikasi Risiko</option>
                                @foreach($riskIdentifications as $riskIdentification)
                                    <option value="{{ $riskIdentification->id }}" {{ old('erkap_risk_identification_id', $riskAnalysis->erkap_risk_identification_id) == $riskIdentification->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_probability_id" class="form-select @error('erkap_risk_probability_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_risk_probability_id', $riskAnalysis->erkap_risk_probability_id) ? '' : 'selected' }}>Pilih Probabilitas</option>
                                @foreach($riskProbabilities as $riskProbability)
                                    <option value="{{ $riskProbability->id }}" {{ old('erkap_risk_probability_id', $riskAnalysis->erkap_risk_probability_id) == $riskProbability->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_impact_id" class="form-select @error('erkap_risk_impact_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_risk_impact_id', $riskAnalysis->erkap_risk_impact_id) ? '' : 'selected' }}>Pilih Dampak</option>
                                @foreach($riskImpacts as $riskImpact)
                                    <option value="{{ $riskImpact->id }}" {{ old('erkap_risk_impact_id', $riskAnalysis->erkap_risk_impact_id) == $riskImpact->id ? 'selected' : '' }}>
                                        {{ $riskImpact->name }}
                                    </option>
                                @endforeach
                            </select>
