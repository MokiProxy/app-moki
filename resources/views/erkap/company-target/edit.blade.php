        ...
        <div class="card shadow-sm">
            <div class="card-body border-bottom bg-light">
                <div class="d-flex align-items-center">
                    ...
                    <a href="{{ route('erkap.company-targets.index') }}" class="btn btn-secondary">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.company-targets.update', $companyTarget->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label fw-bold">Periode RKAP <span class="text-danger">*</span></label>
                            <select name="erkap_rkap_id" class="form-select @error('erkap_rkap_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_rkap_id', $companyTarget->erkap_rkap_id) ? '' : 'selected' }}>Pilih Periode RKAP</option>
                                @foreach($rkaps as $rkap)
                                    <option value="{{ $rkap->id }}" {{ old('erkap_rkap_id', $companyTarget->erkap_rkap_id) == $rkap->id ? 'selected' : '' }}>
                                        {{ $rkap->year }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea name="target" class="form-control @error('target') is-invalid @enderror" rows="3" required>{{ old('target', $companyTarget->target) }}</textarea>
                    ...
                    <div class="mt-4 border-top pt-3">
                        ...
                        <a href="{{ route('erkap.company-targets.index') }}" class="btn btn-secondary">Batal</a>
                    </div>
                </form>
            </div>
        </div>
