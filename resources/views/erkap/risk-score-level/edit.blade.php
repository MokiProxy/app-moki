    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.risk-score-levels.update', $riskScoreLevel->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_probability_id" class="form-select @error('erkap_risk_probability_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_risk_probability_id', $riskScoreLevel->erkap_risk_probability_id) ? '' : 'selected' }}>Pilih Risk Probability</option>
                                @foreach($probabilities as $probability)
                                    <option value="{{ $probability->id }}" {{ old('erkap_risk_probability_id', $riskScoreLevel->erkap_risk_probability_id) == $probability->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_impact_id" class="form-select @error('erkap_risk_impact_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_risk_impact_id', $riskScoreLevel->erkap_risk_impact_id) ? '' : 'selected' }}>Pilih Risk Impact</option>
                                @foreach($impacts as $impact)
                                    <option value="{{ $impact->id }}" {{ old('erkap_risk_impact_id', $riskScoreLevel->erkap_risk_impact_id) == $impact->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="score" class="form-control @error('score') is-invalid @enderror" value="{{ old('score', $riskScoreLevel->score) }}" required min="0" step="1">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="level" class="form-select @error('level') is-invalid @enderror" required>
                                <option value="" disabled {{ old('level', $riskScoreLevel->level) ? '' : 'selected' }}>Pilih Level</option>
                                @foreach(['Low', 'Low To Moderate', 'Moderate', 'Moderate To High', 'High'] as $option)
                                    <option value="{{ $option }}" {{ old('level', $riskScoreLevel->level) == $option ? 'selected' : '' }}>
                                        {{ $option }}
                                    </option>
                                @endforeach
                            </select>
