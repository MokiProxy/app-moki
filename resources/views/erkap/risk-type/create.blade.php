    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.risk-types.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="risk_taxonomy_id" class="form-select @error('risk_taxonomy_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('risk_taxonomy_id') ? '' : 'selected' }}>Pilih Risk Taxonomy</option>
                                @foreach($taxonomies as $taxonomy)
                                    <option value="{{ $taxonomy->id }}" {{ old('risk_taxonomy_id') == $taxonomy->id ? 'selected' : '' }}>
                                        {{ $taxonomy->name }}
                                    </option>
                                @endforeach
                            </select>
