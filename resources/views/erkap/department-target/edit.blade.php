    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.department-targets.update', $departmentTarget->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="erkap_company_target_id" class="form-select @error('erkap_company_target_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_company_target_id', $departmentTarget->erkap_company_target_id) ? '' : 'selected' }}>Pilih Sasaran Perusahaan</option>
                                @foreach($companyTargets as $companyTarget)
                                    <option value="{{ $companyTarget->id }}" {{ old('erkap_company_target_id', $departmentTarget->erkap_company_target_id) == $companyTarget->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="hidden" name="division_id" value="{{ old('division_id', $departmentTarget->division_id) }}">
                            <input type="text" class="form-control" value="{{ $departmentTarget->division->name ?? $userDivision->name }}" disabled>
                            ...
                            <select name="division_id" class="form-select @error('division_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('division_id', $departmentTarget->division_id) ? '' : 'selected' }}>Pilih Divisi</option>
                                @foreach($divisions as $division)
                                    <option value="{{ $division->id }}" {{ old('division_id', $departmentTarget->division_id) == $division->id ? 'selected' : '' }}>
                                        {{ $division->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="erkap_rating_criteria_id" class="form-select @error('erkap_rating_criteria_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_rating_criteria_id', $departmentTarget->erkap_rating_criteria_id) ? '' : 'selected' }}>Pilih Rating Criteria</option>
                                @foreach($ratingCriterias as $ratingCriteria)
                                    <option value="{{ $ratingCriteria->id }}" {{ old('erkap_rating_criteria_id', $departmentTarget->erkap_rating_criteria_id) == $ratingCriteria->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="priority" min="1" class="form-control @error('priority') is-invalid @enderror" value="{{ old('priority', $departmentTarget->priority) }}" placeholder="1 = prioritas tertinggi">
                            @error('priority') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-12">
                            <label class="form-label fw-bold">Target <span class="text-danger">*</span></label>
                            <textarea name="target" class="form-control @error('target') is-invalid @enderror" rows="3" required>{{ old('target', $departmentTarget->target) }}</textarea>
