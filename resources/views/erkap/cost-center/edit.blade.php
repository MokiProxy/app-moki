            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.cost-centers.update', $costCenter->id) }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-12">
                            <div class="form-check form-switch form-check-inline">
                                <input type="checkbox" name="is_centralized" value="1" class="form-check-input @error('is_centralized') is-invalid @enderror" id="is_centralized" {{ old('is_centralized', $costCenter->is_centralized) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-12" id="coordinator-field" style="{{ old('is_centralized', $costCenter->is_centralized) ? '' : 'display: none;' }}">
                            ...
                            <select name="coordinating_division_id" class="form-select @error('coordinating_division_id') is-invalid @enderror">
                                <option value="" disabled {{ old('coordinating_division_id', $costCenter->coordinating_division_id) ? '' : 'selected' }}>Pilih Departemen Koordinator</option>
                                @foreach($divisions as $division)
                                    <option value="{{ $division->id }}" {{ old('coordinating_division_id', $costCenter->coordinating_division_id) == $division->id ? 'selected' : '' }}>
                                        {{ $division->name }}
                                    </option>
                                @endforeach
                            </select>
