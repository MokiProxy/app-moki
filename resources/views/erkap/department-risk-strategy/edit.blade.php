    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.department-risk-strategies.update', $departmentRiskStrategy->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_identification_id" class="form-select @error('erkap_risk_identification_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_risk_identification_id', $departmentRiskStrategy->erkap_risk_identification_id) ? '' : 'selected' }}>Pilih Identifikasi Risiko</option>
                                @foreach($riskIdentifications as $riskIdentification)
                                    <option value="{{ $riskIdentification->id }}" {{ old('erkap_risk_identification_id', $departmentRiskStrategy->erkap_risk_identification_id) == $riskIdentification->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea name="strategy" class="form-control @error('strategy') is-invalid @enderror" rows="3" required>{{ old('strategy', $departmentRiskStrategy->strategy) }}</textarea>
