    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.risk-treatments.update', $riskTreatment->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="erkap_risk_identification_id" id="risk-select" class="form-select @error('erkap_risk_identification_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('erkap_risk_identification_id', $riskTreatment->erkap_risk_identification_id) ? '' : 'selected' }}>Pilih Identifikasi Risiko</option>
                                @foreach($riskIdentifications as $riskIdentification)
                                    <option value="{{ $riskIdentification->id }}" {{ old('erkap_risk_identification_id', $riskTreatment->erkap_risk_identification_id) == $riskIdentification->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="erkap_department_risk_strategy_id" id="strategy-select" class="form-select @error('erkap_department_risk_strategy_id') is-invalid @enderror">
                                <option value="" {{ old('erkap_department_risk_strategy_id', $riskTreatment->erkap_department_risk_strategy_id) ? '' : 'selected' }}>Tidak Ada (Opsional)</option>
                                @foreach($strategies as $strategy)
                                    <option value="{{ $strategy->id }}" data-risk-id="{{ $strategy->erkap_risk_identification_id }}" {{ old('erkap_department_risk_strategy_id', $riskTreatment->erkap_department_risk_strategy_id) == $strategy->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="treatment_type" class="form-select @error('treatment_type') is-invalid @enderror" required>
                                <option value="" disabled {{ old('treatment_type', $riskTreatment->treatment_type) ? '' : 'selected' }}>Pilih Jenis Perlakuan</option>
                                @foreach(\App\Models\Erkap\RiskTreatment::getTreatmentTypes() as $key => $typeLabel)
                                    <option value="{{ $key }}" {{ old('treatment_type', $riskTreatment->treatment_type) == $key ? 'selected' : '' }}>
                                        {{ $typeLabel }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="responsible_party" value="{{ old('responsible_party', $riskTreatment->responsible_party) }}" class="form-control @error('responsible_party') is-invalid @enderror" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="date" name="target_date" value="{{ old('target_date', $riskTreatment->target_date?->format('Y-m-d')) }}" class="form-control @error('target_date') is-invalid @enderror" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                @foreach([
                                    ...
                                    <option value="{{ $statusKey }}" {{ old('status', $riskTreatment->status) == $statusKey ? 'selected' : '' }}>
                                        {{ $statusLabel }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" required>{{ old('description', $riskTreatment->description) }}</textarea>
                            @error('description') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-md-12">
                            <label class="form-label fw-bold">Hasil / Tindak Lanjut</label>
                            <textarea name="result" class="form-control @error('result') is-invalid @enderror" rows="2">{{ old('result', $riskTreatment->result) }}</textarea>
