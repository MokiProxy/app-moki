    ...
    <div class="col-lg-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('erkap.risk-taxonomies.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="risk_appetite_id" class="form-select @error('risk_appetite_id') is-invalid @enderror" required>
                                <option value="" disabled {{ old('risk_appetite_id') ? '' : 'selected' }}>Pilih Risk Appetite</option>
                                @foreach($appetites as $appetite)
                                    <option value="{{ $appetite->id }}" {{ old('risk_appetite_id') == $appetite->id ? 'selected' : '' }}>
                                        {{ $appetite->name }}
                                    </option>
                                @endforeach
                            </select>
